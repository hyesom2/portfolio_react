...
const PORTFOLIO_DATA: portfolioDataTypes[] = [
  ...
  },
  {
    ...
    description: [
      '여행을 좋아하는 사람들을 위한 국내 여행 추천 및 일정 계획을 세울 수 있는 웹앱 입니다.',
      '한국관광공사에서 제공하는 TourAPI 4.0을 사용하여 국내의 지역 기반 관광 정보 데이터를 불러옵니다.',
      '또, 다른 프로젝트에서 사용하였던 pocketbase가 아닌 supabase를 활용해 DB를 구상하여 사용자 인증, 즐겨찾기, 일정 데이터를 효율적으로 관리합니다.',
      '폴짝은 국내 여행을 준비하는 사용자들이 여행지를 검색하고, 일정을 계획하며, 자신의 여행을 관리할 수 있는 국내 여행 정보 제공 웹앱 입니다.',
      'React와 TypeScript, Supabase, Zustand, ShadCN UI를 사용하여 개발 되었습니다.',
      ...
      // '공공데이터포털의 TourAPI를 활용하여 여행 정보를 가져오고, Supabase를 통해 사용자 인증, 즐겨찾기, 일정 데이터를 효율적으로 관리합니다. Zustand를 사용하여 전역 상태를 관리하고, ShadCN UI를 활용하여 일관된 디자인과 UI/UX를 구현했습니다.',
    ],
    period: '2025. 03. 10 ~ 2025.06.13',
    mainImage: '/images/polzzak/polzzak.webp',
    image: [],
    url: 'https://polzzak.vercel.app/',
    ...
    contribution: [
      '기획 및 프로젝트 초기 세팅',
      '컴포넌트 제작 (카테고리, 칩, 헤더, 모달, 콘텐츠 상세, 회원가입의 Step, Progressbar, 검색, 유저 메뉴)',
      '페이지 레이아웃 제작(마이페이지, 폴짝, 지도, 검색/추가/수정)',
      '회원가입 페이지 기능 구현',
      '검색 페이지 기능 구현',
      '콘텐츠 상세 페이지 기능 구현',
      '마이페이지(이메일, 비밀번호, 닉네임 변경) 기능 구현',
      '회원탈퇴, 로그아웃 기능 구현',
    ],
    problem: [
      '현재 React로 제작이 되었는데, supabase의 Auth를 활용할 경우 auth관련 데이터를 변경하려면 서버가 필요합니다.',
      '1번의 문제로 인하여, 회원탈퇴 기능이 제대로 동작하지 않습니다.',
    ],
    solve: [
      'Next.js를 활용하여 제대로 구현이 되지 않은 기능들을 해결하려고 합니다.(현재 독학으로 학습중)',
    ],
  },
];
